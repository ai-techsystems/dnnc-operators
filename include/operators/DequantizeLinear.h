...
public:
  ...
  tensor<T> compute(tensor<T> &a, tensor<T> &a_scale, tensor<T> &a_zero_point) {
    ...
    // Check this when you can take input both int and float tensor together in
    compute function

    if (typeid(a_zero_point)!=typeid(tensor<int>) ||
    typeid(a)!=typeid(tensor<int>)) throw std::invalid_argument("tensor types
    not appropriate for DequantizeLinear operator.");
...
if (a.rank() == 1){
    ...
    // Higher rank is not supported till now
      ...
      return result;
            ...
            throw std::invalid_argument("tensor dimenions not appropriate for
